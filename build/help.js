...
.then(response => response.json())
    ...
    .then(response => for (i = 0; i < response.length; i++) {
        x += response[i];
    }
        ...
        // display data
        document.getElementById('barcode').innerHTML = 'Barcode: ' + response.ean;
        document.getElementById('name').innerHTML = 'Name: ' + response.name;
        document.getElementById('country').innerHTML = 'IssuingCountry: ' + response.IssuingCountry;
